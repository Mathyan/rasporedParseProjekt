...
def get_raspored(driver):
    raspored_html = driver.find_element(By.ID, 'uniRaspored').get_attribute('innerHTML')
...
def extract_raspored(raspored_html):
    ...
    week_number = re.findall(r'Tjedan (\d+)', raspored_html)
    ...
    return int(week_number[0]) ,names[0] , time_cells
...
def main():
    ...
    week, name, raspored_data = get_raspored(driver)
    raspored = Raspored()
    raspored.add_week_data(week , name, raspored_data)
    print(raspored.get_weeks())
    # Quit the driver
    driver.quit()
